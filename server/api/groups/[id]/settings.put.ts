...
export default defineEventHandler(async (event) => {
  try {
    ...
    const updated = await groupsDb.updateSettings(id, updatedSettings);
    ...
    console.error(`Error updating group settings ${event.context.params?.id}:`, error);
    return { success: false, error: error.message || 'Failed to update group settings' };
  }
});