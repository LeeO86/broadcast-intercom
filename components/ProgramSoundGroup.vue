<template>
  ...
  >
    ...
    <div class="flex items-center w-full" v-if="!isMuted">
      ...
      <!-- Program Sound Controls -->
      <div class="relative flex-1 flex flex-col items-center justify-center">
        <div class="text-center mb-4">
          <span class="text-sm font-medium" v-if="isPublisher">You are broadcasting</span>
          <span class="text-sm font-medium" v-else-if="currentPublisher">{{ currentPublisher }} is broadcasting</span>
          <span class="text-sm font-medium text-gray-500" v-else>No one is broadcasting</span>
        </div>
        ...
        <button
          ...
          :disabled="isMuted || currentPublisher"
        >
        ...
        <button
          v-else
          type="button"
          class="w-24 h-24 border-4 border-danger-500 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-danger-500 transition-all duration-200 bg-danger-500 text-white"
          :class="[
            buttonStyle === 'round' ? 'rounded-full' : 'rounded-lg'
          ]"
...
const emit = defineEmits(['volume-change', 'mute-toggle', 'become-publisher', 'stop-broadcasting']);
...
const buttonContainerStyle = computed(() => {
  if (props.isPublisher) {
    return {}; // Use default danger color for broadcasting
  } else {
...
const becomePublisher = () => {
  if (isMuted.value || props.currentPublisher) return;
  emit('become-publisher', props.groupId);
};

// Stop broadcasting
const stopBroadcasting = () => {
  emit('stop-broadcasting', props.groupId);
};
...
const toggleMute = () => {
  ...
  // If muting while publishing, stop publishing
  if (isMuted.value && props.isPublisher) {
    emit('stop-broadcasting', props.groupId);
  }
